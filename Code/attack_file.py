...
import socket
import struct
from create_tcp_packet import TCPPacket
from create_ip_packet import IPPacket
...
tcp_header=TCPPacket(source_ip,destn_ip,int(dport),int(sport),data,int(int(seq)+dataLen),int(int(ack)),int(0))
tcp_header.assemble_tcp_fields()

skt2.sendto(str(ip_header)+str(tcp_header)+struct.pack("!10s",data),(destn_ip,int(dport)))

#skt2.sendto(str(ip_header).encode()+str(tcp_header).encode(),(destn_ip,int(dport)))
#skt2.sendto((str(ip_header)+str(tcp_header))+str(struct.pack("!10s",data.encode())),(destn_ip,dport))
