...
class TCPPacket:
 def __init__(self, src, dst, dport, sport, data, seq, ack, urg_ptr=0):
  self.src_ip = src
  self.dst_ip = dst
  self.dport = dport
  self.sport = sport
  self.data   = data
  self.tcp_seq = seq
  self.tcp_ack_seq = ack
  self.tcp_urg_ptr = urg_ptr	
  self.raw = None
 ...
 def calculate_chksum(self):
  ...
  psh = psh + self.raw + self.data.encode('ascii')
